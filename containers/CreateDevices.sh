...
do
  ...
  docker run -ti --rm --name $deviceName1 -d -e PROVISIONING_REGISTRATION_ID=$deviceName1 -e ENVIRONMENT=Development -e DPS_SECURITY_TYPE="X509CA" -e TRANSPORT_TYPE=$transportType -e DPS_IDSCOPE=$dpsIDScope -e DEVICE_CERTIFICATE_PATH=$x509CertificatePath -e DEVICE_CERTIFICATE_PASSWORD=$x509certificatePassword --network="host" iotsimulator-dps
  echo "Device created: $deviceName1"

  echo "Creating the device: $deviceName2"
  docker run -ti --rm --name $deviceName2 -d -e PROVISIONING_REGISTRATION_ID=$deviceName2 -e ENVIRONMENT=Development -e DPS_SECURITY_TYPE="SymmetricKey" -e TRANSPORT_TYPE=$transportType -e DPS_IDSCOPE=$dpsIDScope -e PRIMARY_SYMMETRIC_KEY=$symmetricKey --network="host" iotsimulator-dps
